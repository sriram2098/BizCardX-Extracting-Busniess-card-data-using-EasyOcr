...
#mycursor.execute("PRAGMA multi_thread = 1", multi=True)
...
def upload_database(image):
    ...
    image_insert = """INSERT INTO card_data (Name, Designation, Company_name, Address, Contact_number,Mail_id,Website_link,Image) 
                    VALUES (%s,%s,%s,%s,%s,%s,%s,%s);"""
    mycursor.execute(image_insert, (name, designation, company_name,address, phone_no, email, link, blobimg))
    mydb.commit()
...
def show_database():
    new_df = pd.read_sql("SELECT * FROM card_data", con=mydb)
    return new_df
